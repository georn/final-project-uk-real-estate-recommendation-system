...
def build_model(property_input_shape, user_input_shape):
    ...
    user_branch = Dense(16, activation='relu', kernel_initializer=HeNormal(), kernel_regularizer=l2(0.01))(user_input)
    user_branch = BatchNormalization()(user_branch)
    user_branch = Dropout(0.3)(user_branch)
    user_branch = Dense(8, activation='relu', kernel_initializer=HeNormal(), kernel_regularizer=l2(0.01))(user_branch)
    user_branch = BatchNormalization()(user_branch)
    user_branch = Dropout(0.3)(user_branch)
    ...
    x = Dense(32, activation='relu', kernel_initializer=HeNormal(), kernel_regularizer=l2(0.01))(combined)
    x = BatchNormalization()(x)
    x = Dropout(0.3)(x)
    x = Dense(16, activation='relu', kernel_initializer=HeNormal(), kernel_regularizer=l2(0.01))(x)
    x = BatchNormalization()(x)
    x = Dropout(0.3)(x)
